...
import com.luckeiros.ticketandroid.feature.event.data.mapper.toEventList
...
internal class EventRepositoryImpl(private val retrofit: Retrofit) : EventRepository {
    ...
    override suspend fun getEvent(cities: String): List<Event> =
        withContext(Dispatchers.IO) {
            val service = retrofit.create(EventService::class.java)
            service.getEvent(cities).toEventList()
        }
}